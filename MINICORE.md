...
The exchange rate is applied to compute `toAmount` before any transaction is posted. Both amounts are truncated (never rounded up) to the target currency's `decimal_places`. All four transactions are recorded in `digitaltwinapp.conversions` with the applied rate for full auditability.
